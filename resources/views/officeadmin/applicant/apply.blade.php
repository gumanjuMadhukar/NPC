                ...
                <div class="card">
                    <div class="card-body conatiner">
                        <span class="text-justify">
                            ...
                            <div class="row mt-3">
                                <form method="post" id="form" action="{{ route('office_admin-applicant-edit-apply') }}">
                                    @csrf
                                                    ...
                                                    @if ($exams->count() > 0)
                                                        @foreach ($exams as $exam)
                                                            <option value="{{ $exam->id }}"
                                                                @if ($exam_apply->exam_id == $exam->id) selected @endif>
                                                                {{ $exam->name }}</option>
                                                        @endforeach
                                                    @endif
                                                </select>
                                                    ...
                                                    @if ($levels->count() > 0)
                                                        @foreach ($levels as $level)
                                                            <option value="{{ $level->id }}"
                                                                @if ($exam_apply->level_id == $level->id) selected @endif>
                                                                {{ $level->name }}</option>
                                                        @endforeach
                                                    @endif
                                                </select>
                                                ...
                                                @if ($programs->count() > 0)
                                                    @foreach ($programs as $program)
                                                        <option value="{{ $program->id }}"
                                                            @if ($exam_apply->program_id == $program->id) selected @endif
                                                            class="{{ $program->level_id }}">{{ $program->name }}</option>
                                                    @endforeach
                                                @endif
                                            </select>
                                        </div>
                                            ...
                                            <div class="col-md-12 col-lg-12">
                                                ...
                                                <div class="drag-container">
                                                    ...
                                                    <div class="drag-area drag-area-voucher_image">
                                                        <div class="dropify-message dropify-message-voucher_image {{ $exam_apply?->voucher_image ? 'd-none' : 'd-block' }}">
    ...
    @section('footer-scripts')
        @include('officeadmin.applicant.js.apply')
    @endsection
