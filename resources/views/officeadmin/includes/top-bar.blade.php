...
$user = Auth::guard('office_admin')->user();
@endphp
<div class="navbar-custom">
    <ul class="list-unstyled topnav-menu float-end mb-0">
        <li>
            <a class="nav-link">
               {{Auth::guard('office_admin')->user()->name}}  (<b>{{Auth::guard('office_admin')->user()->name}}</b>)
            </a>
        </li>
        <li>
            <a class="nav-link" href="{{route('office_admin-logout')}}">
