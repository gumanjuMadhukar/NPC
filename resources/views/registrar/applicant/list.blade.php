                            ...
                            <div class="row">
                                <div class="col-md-2 col-sm-6 col-xs-12">
                                    <select class="form-select select2" name="exam_id">
                                        <option value="0" @if ($exam_id == 0) selected @endif>All
                                        </option>
                                        ...
                                        @if ($exams->count() > 0)
                                            @foreach ($exams as $exam)
                                                <option value="{{ $exam->id }}"
                                                    @if ($exam->id == $exam_id) selected @endif>{{ $exam->name }}
                                                </option>
                                            @endforeach
                                        @endif
                                    </select>
                                ...
                                <div class="col-md-2 col-sm-6 col-xs-12">
                                    <select class="form-select select2" name="level_id">
                                        <option value="0" @if ($level_id == 0) selected @endif>All Levels
                                        </option>
                                        @if ($levels->count() > 0)
                                            @foreach ($levels as $level)
                                                <option value="{{ $level->id }}"
                                                    @if ($level->id == $level_id) selected @endif>
                                ...
                                {{-- <div class="col-md-2 col-sm-6 col-xs-12">
                                    <select class="form-select select2" name="college_name">
                                        <option value="" @if ($college_name == '') selected @endif>All
                                            Colleges</option>
                                        @if ($colleges->count() > 0)
                                            @foreach ($colleges as $college)
                                                <option value="{{ $college->name }}"
                                                    @if ($college->name == $college_name) selected @endif>{{ $college->name }}
                                                </option>
                                            @endforeach
                                        @endif
                                    </select>
                                ...
                                <div class="col-md-2 col-sm-6 col-xs-12">
                                    <select class="form-select select2" name="role_name">
                                        <option value="" @if ($role_name == '') selected @endif>All States
                                        </option>
                                        @if ($roles->count() > 0)
                                            @foreach ($roles as $role)
                                                <option value="{{ $role->auth_name }}"
                                                    @if ($role->name == $role_name) selected @endif>{{ $role->name }}
                                                </option>
                                            @endforeach
                                        @endif
                                    </select>
                                ...
                                <div class="col-md-2 col-sm-6 col-xs-12">
                                    <select class="form-select select2" name="status">
                                        ...
                                        <option value="progress" @if ($status == 'progress') selected @endif>Progress
                                        </option>
                                        <option value="accepted" @if ($status == 'accepted') selected @endif>
                                            Accepted</option>
                                        <option value="re-exam" @if ($status == 're-exam') selected @endif>Re-exam
                                        </option>
                                        <option value="pending" @if ($status == 'pending') selected @endif>Pending
                                        </option>
                                        <option value="onhold" @if ($status == 'onhold') selected @endif>On-hold
                                        </option>
                                        <option value="re-check" @if ($status == 're-check') selected @endif>
                                            Re-check</option>
                                ...
                                <table class="table mb-0">
                                    ...
                                    <tbody>
                                        @foreach ($result['applicants'] as $applicant)
                                            <tr>
                                                <td>{{ $result['count']++ }}</td>
                                                <td>{{ $applicant->user_info->first_name }}
                                                    {{ $applicant->user_info->middle_name }}
                                                    {{ $applicant->user_info->last_name }}</td>
                                    ...
                                    @elseif ($applicant->status == 'pending')background-color: grey; @endif
                                    color:#fff;
                                    padding:3px;
                                    border-radius:4px;
                                    ">{{ ucwords($applicant->status) }}</span>
                                                </td>
                                                <td>{{ $applicant->state }}</td>
                                                <td>
                                                    <a href="{{ route('registrar-applicant-profile', $applicant->user->id) }}"
