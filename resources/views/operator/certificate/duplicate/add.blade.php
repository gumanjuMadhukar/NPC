...
@section('content')
    @php
        ...
        [$certificate_program, $board_university, $passed_year] = array_pad(explode(':', $qualification), 3, '');
        // dd($board_university);
        if (count(explode(':', $qualification)) > 3) {
            // Handle unexpected data here, like logging or discarding extra parts
            // You can log it for debugging or trim the array to 3 elements
            $qualification_parts = explode(':', $qualification);
            $qualification_parts = array_slice($qualification_parts, 0, 3); // Keep only the first 3 parts
            [$program, $board_university, $passed_year] = $qualification_parts;
        }

        // dd($program,$board_university);
    ...
    <div class="content">
        <div class="container-fluid">
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-body">
                        <form method="POST" action="{{ route('operator-certificate-duplicate-store') }}" id="form">
                            @csrf
                            <input type="hidden" name="id" value="{{ $row ? $row->id : 0 }}">
                            <input type="hidden" name="user_info_id" value="{{ $row ? $row->user->info->id : 0 }}">
                                        ...
                                        <div class="row">
                                            <div class="form-group col-md-12 col-lg-12">
                                                <div class="drag-container">
                                                    <button type="button"
                                                        class="{{ $row?->user->info->profile_picture ? 'd-block' : 'd-none' }} img-delete-btn"
                                                    ...
                                                    <div class="drag-area drag-area-profile_picture" style= "height:120px;">
                                                        <div
                                                            class="dropify-message dropify-message-profile_picture {{ $row?->user->info->profile_picture ? 'd-none' : 'd-block' }}">
                                                        ...
                                                        <input name="profile_picture" type="hidden" id="profile_picture"
                                                            value="{{ $row?->user->info->profile_picture ?? old('profile_picture') }}"
                                                            class="file-input" />
                                                        <div class="image-preview">
                                                            @if ($row?->user->info->profile_picture)
                                                                <img src="{{ $row->user->info?->full_profile_picture }}"
                        ...
                        ">
                                Pursuant to the decision dated <input type="text" name="decision_date"
                                    value="{{ $row ? $row->decision_date : old('decision_date') }}" /> of the Council, the
                                name of
                                <span style="font-size: 26px; font-weight: 600;">
                                    <input type="text" name="name" value="{{ $row ? $row->name : old('name') }}" />
                                </span> date of birth
                                ...
                                <span style="font-size: 26px; font-weight: 700;">
                                    <input type="text" name="ward_no"
                                        value="{{ $row ? $ward_no : old('ward_no') }}" />
                                </span> of
                                <span style="font-size: 26px; font-weight: 700;">
                                    <input type="text" name="municipality"
                                        value="{{ $row ? $municipality : old('municipality') }}" />
                                </span>
                            ...
                            <tr style="text-align: center;">
                                ...
                                <td style=" border: 1px solid black; text-align: center;  font-size: 20px; font-weight: bold;">
                                    <input type="text" name="program_id" value ="{{$qualification ? $program['qualification'] : old('qualification')}}" /></td>
                                </td>
                                <td style=" border: 1px solid black; text-align: center;  font-size: 20px; font-weight: bold;"> <input type="text" name="board_university" value ="{{$qualification ? $board_university : old('board_university')}}"/></td>
                                <td style=" border: 1px solid black; text-align: center;  font-size: 20px; font-weight: bold;"> <input type="text" name="passed_year" value ="{{$qualification ? $passed_year : old('passed_year')}}"/></td>
                            </tr>
                        </table> --}}
                            <table style="text-align: center; border-collapse: collapse; width: 100%;">
                                <tr style="text-align: center;">
                                ...
                                <tr style="text-align: center;">
                                    ...
                                    <td
                                        style=" border: 1px solid black; text-align: center;  font-size: 20px; font-weight: bold;">
                                            <select class="form-select select2" name="program_code" style="min-width: 300px;">
                                                @foreach ($programs as $program)
                            ...
                            <div class="" style="text-align: center;">
                                <span
                                    ...
                                    - This certificate should be updated in every <input type="text" name="duration"
                                        value ="5" /> , from the date of issue.</span>
                            </div>
...
@section('footer-scripts')
    @include('operator.certificate.duplicate.js.add')
@endsection
