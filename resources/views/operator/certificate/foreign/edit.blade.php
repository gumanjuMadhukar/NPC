    ...
    <div class="container-fluid">
        ...
        <div class="col-lg-12">
            ...
            <div class="collapse" id="collapseExample">
                <div class="card">
                    <div class="card-body">
                        <form enctype="multipart/form-data" method="post" action="{{route('operator-program-store')}}" id="form2">
                            @csrf
        ...
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <form method="POST" action="{{ url('operator/certificate/foreignstore') }}" id="form">
                        @csrf
                        ...
                        <div id="container" style=" margin-top: 22px; display: flex; justify-content: space-between; align-items: center;">
                                    ...
                                    <div class="row">
                                        <div class="form-group col-md-12 col-lg-12">
                                            <div class="drag-container">
                                                <button type="button" class="{{ ($certificate->user->info?->profile_picture) ? 'd-block' : 'd-none' }} img-delete-btn" onclick="imageDelete('profile_picture')" id="btn_profile_picture_delete"><i class="fa fa-times"></i></button>
                                                <div class="drag-area drag-area-profile_picture" style="height:116px">
                                                    <div class="dropify-message dropify-message-profile_picture {{ ($certificate->user->info?->profile_picture) ? 'd-none' : 'd-block' }}">
                                                        <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" x="0px" y="0px" width="64px" height="64px" viewBox="0 0 64 64" enable-background="new 0 0 64 64" xml:space="preserve">
                                                            <path fill="none" stroke="#8a8a8a" stroke-width="2" stroke-miterlimit="10" d="M41,50h14c4.565,0,8-3.582,8-8s-3.435-8-8-8  c0-11.046-9.52-20-20.934-20C23.966,14,14.8,20.732,13,30c0,0-0.831,0-1.667,0C5.626,30,1,34.477,1,40s4.293,10,10,10H41" />
                                                            <polyline fill="none" stroke="#8a8a8a" stroke-width="2" stroke-linejoin="bevel" stroke-miterlimit="10" points="23.998,34   31.998,26 39.998,34 " />
                                                            <g>
                                                                <line fill="none" stroke="#8a8a8a" stroke-width="2" stroke-miterlimit="10" x1="31.998" y1="26" x2="31.998" y2="46" />
                                                            </g>
                                                        </svg>
                                                        <p>Click here to upload image</p>
                                                    </div>
                                                    <input name="profile_picture" type="hidden" id="profile_picture" value="{{ $certificate->user->info->profile_picture ??  old('profile_picture') }}" class="file-input" />
                                                    <div class="image-preview p-0"> @if($certificate->user->info?->profile_picture) <img  style="object-fit:contain;width:100%; object-position:center; height:auto;"src="{{ $certificate->user->info?->full_profile_picture }}" id="display_profile_picture" class="preview-image"> @else <img src="" id="display_profile_picture" class="d-none preview-image">
                                                        @endif
                                                    </div>
                                                </div>
                            ...
                            <div class="col cert_name" id="col2" style="
                            padding-left: 0.3rem;
                            font-size: 30px;
                            font-weight: 600;">
                        ...
                        line-height: 1;
                        ">
                            Pursuant to the decision dated  <input type="text" name="decision_date" value="{{date('d-m-Y',strtotime($certificate['decision_date']))}}" /> of the Council, the name of
                            <span style="font-size: 26px; font-weight: 600;">
                                <input type="text" name="first_name" value="{{ $certificate->user->info->first_name}}" />
                                <input type="text" name="middle_name" value="{{ $certificate->user->info->middle_name}}" />
                                <input type="text" name="last_name" value="{{ $certificate->user->info->last_name}}" />
                            </span> date of birth
                            <input type="text" name="dob_nep" value="{{ $certificate->user->info->dob_nep}}" />
                            a resident ward No.
                            <span style="font-size: 26px; font-weight: 700;">
                                <input type="text" name="ward_no" value="{{ $certificate->user->info->ward_no}}" />
                            </span> of
                            <span style="font-size: 26px; font-weight: 700;">
                                <select name="municipality_id" style="min-width: 300px;">
                                    @foreach($municipalities as $municipality)
                                    <option value="{{ $municipality->id }}" @if($municipality->id == $certificate->user->info->municipality_id) selected @endif >{{ $municipality->name }}</option>
                                    @endforeach
                                </select>
                            </span>
                            Metropolitan City /Sub-Metropolitan City /Municipality /Rural Municipality
                            <span style="font-size: 26px; font-weight: 700;">
                                <select name="district_id" style="min-width: 300px;">
                                    @foreach($districts as $district)
                                    <option value="{{ $district->id }}" @if($district->id == $certificate->user->info->district_id) selected @endif >{{ $district->name }}</option>
                                    @endforeach
                                </select>
                            </span>
                            District
                            <span style="font-size: 26px; font-weight: 700;">
                                <select name="province_id" style="min-width: 300px;">
                                    @foreach($provinces as $province)
                                    <option value="{{ $province->id }}" @if($province->id == $certificate->user->info->province_id) selected @endif >{{ $province->name }}</option>
                                    @endforeach
                                </select>
                            </span>
                            Province is registered as
                            <span style="font-size: 26px; font-weight: 700;">
                                <select name="program_id" style="min-width: 300px;">
                                    @foreach($programs as $program)
                                    <option value="{{ $program->id }}" @if($program->id == $certificate->program_id) selected @endif >{{ $program->code }}</option>
                                    @endforeach
                                </select>
                            </span>
                            of
                            <span style="font-size: 26px; font-weight: 700;">
                                <select name="level_id" style="min-width: 300px;">
                                    @foreach($levels as $level)
                                    <option value="{{ $level->id }}" @if($level->id == $certificate->level_id) selected @endif >{{ $level->short_name_english }}</option>
                                    @endforeach
                                </select>
                            </span>
                        ...
                        <div class="" style="height: 130px; display: block; ">
                            <div class="left" style="font-weight: 700; line-height: 1.2;   text-align: left; font-size:18px ;float: left;">
                                Registration No:
                                <span style="font-size: 26px;">
                                    <input type="text" name="cert_registration_number" value="{{$certificate['cert_registration_number']}}" />
                                </span>
                                <br>
                                Date of issue:
                                <span style="font-size: 22px;">
                                    <input type="text" name="issued_date" value="{{date('d-m-Y',strtotime($certificate['issued_date']))}}" />
                                </span>
                                <br>
                                </span><br>
                                Seal of the Council:
                        ...
                        <table style="text-align: center; border-collapse: collapse; width: 100%;">
                            <tr style="text-align: center;">
                                ...
                                <th style="text-align: center;font-size:14px ;font-weight: bold;">Institution / University / Board</th>
                                <th style="text-align: center; font-size:14px ;font-weight: bold;">Passed Year</th>
                            </tr>
                            @php $key = 0; @endphp
                            @foreach($qualifications as $key => $qualification)
                            <tr style="text-align: center;">
                                <input type="hidden" name="qualification[{{$key}}][id]" value="{{$qualification['id']}}">
                                <td style=" border: 1px solid black; text-align: center;  font-size: 20px; font-weight: bold;">{{ $key + 1 }}</td>
                                <td style=" border: 1px solid black; text-align: center;  font-size: 20px; font-weight: bold;">
                                    <select name="qualification[{{$key}}][program_id]" style="min-width: 300px;">
                                        @foreach($programs as $program)
                                        <option value="{{ $program->id }}" @if($program->id == $certificate->program_id) selected @endif >{{ $program->code }}</option>
                                        @endforeach
                                    </select>
                                ...
                                <td style=" border: 1px solid black; text-align: center;  font-size: 20px; font-weight: bold;">
                                    <input type="text" name="qualification[{{$key}}][board_university]" value="{{@$qualification['board_university']}}" />
                                </td>
                                <td style=" border: 1px solid black; text-align: center;  font-size: 20px; font-weight: bold;">
                                    <input type="text" name="qualification[{{$key}}][passed_year]" value="{{@$qualification['passed_year']}}" />
                                </td>
                            </tr>
                            @endforeach
                            <tr style="text-align: center;">
                                ...
                                <td style=" border: 1px solid black; text-align: center;  font-size: 20px; font-weight: bold;"> <input type="text" name="board_university" /></td>
                                <td style=" border: 1px solid black; text-align: center;  font-size: 20px; font-weight: bold;"> <input type="text" name="passed_year" /></td>
                            </tr>
                            <tr style="text-align: center;">
                                <td style=" border: 1px solid black; text-align: center;  font-size: 20px; font-weight: bold;">{{ ++$key + 1 }}</td>
                                <td style=" border: 1px solid black; text-align: center;  font-size: 20px; font-weight: bold;">
