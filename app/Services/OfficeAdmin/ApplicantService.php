...
use App\Exports\OfficeAdmin\ApplicantExport;
use App\Exports\OfficeAdmin\ApplicantProgramExport;
use App\Jobs\OfficeAdmin\ExamApplyStatusJob;
...
class ApplicantService
{
    ...
    public function list($per_page, $page, $q, $exam_id, $level_id, $status, $program_id, $college_name, $state_name)
    {
        ...
        try {
            // $query = ExamApply::select('*')->where('state', 'office_admin');
    ...
    public function myList($per_page, $page, $q, $exam_id, $level_id, $status, $program_id, $college_name, $state_name)
    {
        ...
        try {
            // $query = ExamApply::select('*')->where('state', 'office_admin');
            $query = ExamApply::select('exam_applies.*')
                ->where('exam_applies.state', 'office_admin');
            if ($q) {
                $query->where(function ($qry) use ($q) {
                    $qry->whereHas('user', function ($qry) use ($q) {
                        $qry->where(function ($query) use ($q) {
    ...
    public function status($request)
    {
        try {
            if ($request['status'] === "accepted") {
                ...
                $status = 'rejected';
                $state = 'office_admin';
                $remarks = (!empty($request['remarks'])) ? $request['remarks'] : 'Rejected By Computer OfficeAdmin';
                $exam_log_remarks = (!empty($request['remarks'])) ? $request['remarks'] : 'Rejected By Computer OfficeAdmin';
                $subject = 'Application Form Status Update';
            } elseif ($request['status'] === "onhold") {
                $status = 'onhold';
                $state = 'office_admin';
                $remarks = (!empty($request['remarks'])) ? $request['remarks'] : 'Hold By Computer OfficeAdmin';
                $exam_log_remarks = (!empty($request['remarks'])) ? $request['remarks'] : 'Hold By Computer OfficeAdmin';
            } else {
                $status = 'pending';
                $state = 'office_admin';
                $remarks = (!empty($request['remarks'])) ? $request['remarks'] : 'Pending By Computer OfficeAdmin';
                $exam_log_remarks = (!empty($request['remarks'])) ? $request['remarks'] : 'Pending By Computer OfficeAdmin';
            }
            ...
            $exam_log->system_user_id = Auth::guard('office_admin')->id();
            $exam_log->status = $request['status'];
            $exam_log->state = 'office_admin';
    ...
    public function forwardReExam($request)
    {
        try {
            ...
            $exam_applies = ExamApply::where('state', 'office_admin')->where('status','re-exam')->get();
    ...
    function selectedForwardReExam($request) {
        try {
            ...
            $exam_applies = ExamApply::where('state', 'office_admin')->whereIn('id', $ids)->where('status','re-exam')->get();
            foreach ($exam_applies as $exam_apply) {
                $exam_apply->status = $status;
                $exam_apply->state = $state;
                $exam_apply->remarks = $remarks;
