...
class DashboardService
{
    ...
    public function statusWiseStudents($per_page, $page, $q, $status, $exam_id)
    {

        try {
            $query = ($status && $status === "re-exam")
            ? ExamApply::where('exam_id', $exam_id)->where('attempt', '>', 1)
            : ExamApply::where('exam_id', $exam_id)->when($status !== null, fn($q) => $q->where('status', $status));
            if ($q) {
                $query->where(function ($qry) use ($q) {
                    $qry->whereHas('user', function ($qry) use ($q) {
                        $qry->where(function ($q2) use ($q) {
            ...
            $data['status_wise_students'] = $query->orderBy('id', 'asc')->paginate($per_page);
            $data['status_wise_students']->appends(array('q' => $q));
            if ($page != 1) {
                $data['total_data'] = $data['status_wise_students']->total();
                $data['count'] = ($per_page * $page) - $per_page + 1;
                $data['from_data'] = $data['count'];
                $to_data = $page * $data['status_wise_students']->count();
                $data['to_data'] = ($to_data > $data['from_data']) ? $to_data : $data['total_data'];
            } else {
                $data['total_data'] = $data['status_wise_students']->total();
                $data['count'] = 1;
                $data['from_data'] = 1;
                $data['to_data'] = $data['status_wise_students']->count();
            }
            return $data;
        } catch (\Exception $e) {
            return response()->json(['error' => $e->getMessage()], 400);
