...
class MoveExamCommitteeJob implements ShouldQueue
{
    ...
    public function handle()
    {
       ...
       $exam_applies = ExamApply::where(['state' => 'subject_committee', 'status' => 'progress'])->whereNotNull('exam_id')->whereIn('program_id', $program_ids)->get();
       if($exam_applies->count() > 0) {
        foreach ($exam_applies as $exam_apply) {
            ...
            if ($exam_apply->subject_committee_count >= $average) {
                ...
                $exam_log_remarks = 'Profile Verified and forwarded to exam committee';
