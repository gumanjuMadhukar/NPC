...
class DashboardController extends Controller
{
    ...
    public function examDetail($id)
    {
        ...
        $data['operator_student_count'] = ExamApply::where('exam_id', $id)->whereIn('status', ['progress', 're-exam'])->count();
        $data['operator_accepted_count'] = $this->countOperatorExamLogs($id, 'accepted', );
        $data['operator_rejected_count'] = ExamApply::where(['exam_id' =>$id, 'state'=>'operator', 'status'=>'rejected'])->count();
        ...
        return view('operator.dashboard.exam_detail', $data);
    }
    public function programWiseStudents(Request $request, $program_id, $exam_id)
    {
        ...
        return view('operator.dashboard.program_detail', $data);
    }
    public function statusWiseList(Request $request, $program_id, $exam_id)
    {
        ...
        return view('operator.dashboard.status_wise_detail', $data);
    }

    private function countOperatorExamLogs($examId, $status)
    {
        return ExamLog::where('state', 'operator')
            ->where('status', $status)
