...
use App\Exports\AdmitCardReader\ApplicantExport;
...
use App\Services\AdmitCardReader\DashboardService;
...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        $page = $request->page ?? 1;
        $data['q'] = $request->q ?? '';
        $data['user'] = 'admincardreader';
        $data['result'] = $this->service->list($per_page, $page, $data['q']);
        return view('admitcardreader.dashboard.index', $data);
    }
    public function examDetail($id)
    {
        ...
        $data['officer_student_count'] = ExamApply::where('exam_id', $id)->whereIn('status', ['progress', 're-exam'])->count();
        $data['officer_accepted_count'] = $this->countOfficerExamLogs($id, 'accepted', );
        $data['officer_rejected_count'] = ExamApply::where(['exam_id' => $id, 'state' => 'officer', 'status' => 'rejected'])->count();
        ...
        return view('admin.dashboard.exam_detail', $data);
    }
    public function programWiseStudents(Request $request, $program_id, $exam_id)
    {
        ...
        return view('admin.dashboard.program_detail', $data);
    }

    private function countOfficerExamLogs($examId, $status)
    {
    ...
    //     return Excel::download(new ApplicantExport($request->exam_id), Str::slug(Str::words($exam->name, 3, '')) . '.xlsx');
    // }
